...
export class Segment2dApiRepository implements Segmen2dRepository {
  ...
  async show(id: string): Promise<Segment2D[]> {
    ...
    return data.data.map((item) =>
      Segment2D.create({
        name: item.part_weight,
        measurements: (item.std_measurement?.special_accept_segments || []).map(
          (measurement) =>
            Measurement.create({
              ...
              judgement: measurement.cavity_results?.actual_result_judgement,
              saResult: measurement.cavity_results?.sa_result,
              saJudgement: measurement.cavity_results?.sa_result_judgement,
              checked: false,
            })
        ),
      })
