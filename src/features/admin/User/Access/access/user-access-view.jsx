...
import { PenAltIcon, TrashIcon, MenuIcon } from "@common/components/icons";
        ...
        <div>
          <table className="w-full">
            <thead className="bg-[#FAFAFB] border-y border-[#D0D3D9]">
              <tr>
                <th className="py-6 text-start pl-10 text-[#514E4E] font-[500]">Role</th>
                <th className="py-6 text-start pl-80 text-[#514E4E] font-[500]">Action</th>
              </tr>
            </thead>
            <tbody>
              {model.access.data.map((item, index) => (
                <tr className="border-b border-[#D0D3D9]" key={index}>
                  <td className="py-4 text-start pl-10 text-[#514E4E] ">{item.name}</td>
                  <td className="py-2 pl-80 text-[#514E4E] flex gap-3 justify-start">
                    <button onClick={() => model.toMenu(item.id)}
