...
export function useAccount() {
  ...
  const onDetail = (e: React.MouseEvent<HTMLButtonElement>, id: User["id"]) => {
    e.preventDefault();
  ...
  const onDelete = (e: React.MouseEvent<HTMLButtonElement>, id: User["id"]) => {
    e.preventDefault();
  ...
  const onConfirmDelete = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    userRepo.delete(account.data.find((item) => item.checked).id).then(() => {
      setAccount((prevState) => {
        const newState = prevState.duplicate();
