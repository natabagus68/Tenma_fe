...
const MaterialFormView = () => {
  ...
  return (
    <div className="font-open-sans text-[#514E4E]">
      <Breadcrumbs
        items={["Material", materialForm.id ? "Edit Data" : "Add Data"]}
      />
      <div className="m-auto w-full border border-[#D0D3D9] rounded-md">
        ...
        <form className="border-t border-[#D0D3D9] px-8 py-6">
          ...
          <div className="flex flex-col gap-2 mt-4">
            ...
            <Select
              className="w-[80%] rounded-lg outline-none text-md text-sm font-400"
              value={materialForm.materialColor}
              options={materialForm.colors.data.map((item) => ({
                value: item,
                label: item.materialColor,
              }))}
              onChange={materialForm.handleSelect}
            />
          </div>
