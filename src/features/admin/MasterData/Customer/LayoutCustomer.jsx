...
const LayoutCustomer = () => {
  ...
  function getLastSegmentOfUrl(url) {
    ...
    return segments[segments.length -1] !== "" ? segments[segments.length - 1] : segments[segments.length - 2]
  }
  return (
    <>
      ...
      <div className="m-auto w-full border border-[#D0D3D9] rounded-md pb-6">
        ...
        <div className="w-full py-4 px-8 flex justify-between items-center border-t border-[#D0D3D9]">
          <div className="flex gap-5">
            <Link to={"/master-data/customer"}
              className={`text-[#514E4E] font-[400] hover:text-[#F04438] hover:border-b pb-2 hover:border-[#F04438]
              ${ getLastSegmentOfUrl(url) === "customer" ? "border-b-2 pb-2 border-[#F04438] text-[#F04438]" : null }`}
            >Customer</Link>
            <Link to={"/master-data/customer/customer-model"}
              Model
              className={`text-[#514E4E] font-[400] hover:text-[#F04438] hover:border-b-2 pb-2 hover:border-[#F04438]
              ${ getLastSegmentOfUrl(url) === "customer-model" ? "border-b-2 pb-2 border-[#F04438] text-[#F04438]" : null }`}
            >Customer Model</Link>
            <Link
              to={"/master-data/customer/customer-model-group"}
              className={`text-[#514E4E] font-[400] hover:text-[#F04438] hover:border-b-2 pb-2 hover:border-[#F04438]
              ${ getLastSegmentOfUrl(url) === "customer-model-group" ? "border-b-2 pb-2 border-[#F04438] text-[#F04438]" : null }`}
          ...
          <button
            ...
            onClick={(e) => {
              ...
              getLastSegmentOfUrl(url) === "customer"
                ? navigate("add-data-c1") : getLastSegmentOfUrl(url) === "customer-model"
                ? navigate("add-data-c2") : navigate("add-data-c3")
            }}
          >+ Add Data</button>
        </div>
        <div>
