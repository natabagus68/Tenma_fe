        ...
        <>
          ...
          >
            <div className="w-full py-5 px-12 flex justify-between gap-5 items-center">
              ...
              <div className="">
                <button
                  onClick={(e) => model.deleteTableUpdate(e, index)}
          ...
          <div>
            <table className="w-full border border-gray-100 rounded-b-lg">
              ...
              <tbody>
                {item.measurements.map((measurement, index2) => (
                  <tr key={item.id}>
                    <td className="border bg-white text-center py-5">
                      {measurement.character}
                    </td>
                    <td className="border bg-white text-center py-5">
                      {measurement.nominal == "Decimal"
                        ? `${measurement.nominal} (${measurement.nominalValue})`
                        : measurement.nominal}
                    </td>
                    <td className="border bg-white text-center py-5">
                      {measurement.upper}
                    ...
                    <td className="border bg-white ">
                      <div className="w-full flex justify-between">
                        <div className="py-5 pr-3 h-full text-start w-[50%] border-r border-gray-100 pl-2">
                          {model.segments[index].measurements[
                            index2
                          ].nominal.toLowerCase() !== "decimal" ? (
                            <input
                              type="text"
                              value={
                                model.segments[index].measurements[index2]
                                  .result
                              }
                              name="result"
                              onChange={(e) =>
                                model.onChangeInput(e, index, index2)
                              }
                        ...
                        <div className=" w-full text-center items-center flex justify-center">
                          <p
                            className={`m-auto ${
                              model.segments[index].measurements[
                                index2
                              ].judgement.toLowerCase() == "ok" ||
                              model.segments[index].measurements[
                                index2
                              ].judgement.toLowerCase() == "good"
                                ? "text-black"
                                : "text-red-500"
                            }`}
                          >
